...
pub mod example_usage {
    ...
    pub fn serve_asset(path: &str) -> Option<(Vec<u8>, &'static str)> {
        match path {
            "/icon.png" => Some((crate::images::ICON_PNG.to_vec(), "image/png")),
            "/logo.svg" => Some((crate::images::LOGO_SVG.to_vec(), "image/svg+xml")),
            "/banner.txt" => Some((
                crate::art::rust::RUST_BANNER.as_bytes().to_vec(),
                "text/plain",
            )),
            "/rust-logo.txt" => Some((
                crate::art::rust::RUST_LOGO.as_bytes().to_vec(),
                "text/plain",
            )),
            _ => None,
        }
    ...
    pub fn print_asset_info() {
        ...
        println!("{}", crate::images::ICON_PNG_INFO);
        println!("{}", crate::images::LOGO_SVG_INFO);

        println!("\n=== Text Assets ===");
        println!("{}", crate::art::rust::RUST_LOGO_INFO);
        println!("{}", crate::art::rust::RUST_BANNER_INFO);

        println!("\n=== Runtime Statistics ===");
        println!(
            "RUST_LOGO: {} lines, {} chars",
            crate::art::get_line_count(crate::art::rust::RUST_LOGO),
            crate::art::get_char_count(crate::art::rust::RUST_LOGO)
        );
        println!(
            "BANNER: {} lines, {} chars",
            crate::art::get_line_count(crate::art::rust::RUST_BANNER),
            crate::art::get_char_count(crate::art::rust::RUST_BANNER)
        );
    }
}
